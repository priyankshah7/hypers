...
from hypers.tools._types import DecomposeType, DECOMPOSE_TYPES
...
class _decompose:
    ...
    def calculate(self, mdl: DecomposeType,
                  plot: bool = False,
                  figsize: tuple = None,
        ...
        self.mdl = mdl
        n_components = self.mdl.get_params()['n_components']
        images = self.mdl.fit_transform(self.X.flatten()).reshape(self.X.data.shape[:-1] + (n_components,))
        specs = self.mdl.components_.transpose()
