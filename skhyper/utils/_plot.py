...
def _plot_decomposition(plot_range, images, spectra, dim=3):
    if not isinstance(plot_range, tuple):
        raise TypeError('plot_range must be a tuple containing two elements e.g. plot_range=(0, 5)')

    if not len(plot_range) == 2:
        raise ValueError('plot_range must be a tuple containing two elements e.g. plot_range=(0, 5)')
    ...
    if dim == 3:
        ...
        for component in range(plot_range[0], plot_range[1]):
            ...
            im = plt.imshow(images[:, :, component])
